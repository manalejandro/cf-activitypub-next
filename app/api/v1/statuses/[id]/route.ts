...
export async function GET(
  ...
  const isFollowing = authActor ? await isAcceptedFollower(env.DB, authActor.id, obj.actorId) : false;
  ...
  const [quotesCount, quote, filtered, authorLastStatusAt, authorExtras, bookmarked, muted, authorFields] = await Promise.all([
    ...
    authActor ? getMutedActorIds(env.DB, authActor.id).then((ids) => new Set(ids).has(obj.actorId)) : Promise.resolve(false),
