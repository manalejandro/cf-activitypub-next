...
- **API docs**: `npm run generate:openapi` (runs on predev/prebuild/prepreview/predeploy) writes `lib/api-docs/openapi.json` for the Swagger UI; `npm run check:openapi` fails when the committed spec is stale. The generator is **self-contained** (no metadata file): it scans `app/**/route.ts` and derives paths/methods, tags from the path, query params (`searchParams.get`), request bodies and the file's leading comment as description. Security is read from the handlers: everything under `/api/**/admin` is locked (administrator/moderator role), as is any handler that resolves the caller (`getAuthenticatedActor`, `getAdminRole`, manual `Bearer` parsing) and rejects a missing one (`if (!actor) return unauthorized()`, `role !== "admin"`). Endpoints that only read the viewer optionally (public timelines), login/registration endpoints and HTTP-signature inboxes stay public. Handlers that delegate (`PATCH → PUT`) or are re-exported inherit the target's guard. **Response schemas are generated from the TypeScript types** (`lib/types/index.ts` plus the declared return types of the `lib/mastodon` serializers, converted with the TypeScript compiler), and each operation is wired to the schema of the serializer it calls (arrays for `.map()`/timeline builders, per-property refs for envelope responses). `lib/__tests__/openapi.test.ts` asserts the committed file matches the source and pins the lock/public and schema matrices.
