...
- Hooks: `enqueueMediaCache` in `handleCreate`/`saveObjectAttachments`/`storeObjectAttachments`/remote featured items and in `upsertRemoteActor` for avatars/headers. Cron runs `processMediaCacheQueue` (concurrent) + `maintainMediaCache` (expiry + byte-budget eviction, daily byte recount in KV `mediacache:bytes`). Maintenance is FIFO: when a limit is reached the OLDEST entries (`fetched_at ASC`) are replaced, deletions are capped per tick, and `MEDIA_CACHE_MIN_ENTRIES` (20) is a floor — the cache is never wiped automatically (only the full-admin purge wipes). Admin API: `GET/DELETE /api/v1/admin/media_cache`.
