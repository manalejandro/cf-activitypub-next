...
- Hooks: `enqueueMediaCache` in `handleCreate`/`saveObjectAttachments`/`storeObjectAttachments`/remote featured items and in `upsertRemoteActor` for avatars/headers. Cron runs `maintainMediaCache` (expiry + byte-budget eviction) BEFORE `backfillMediaCache`/`processMediaCacheQueue`, so the size limit is enforced before new objects are added. Eviction is FIFO (`fetched_at ASC`, oldest replaced first) with a 500 MB/tick byte budget — enough to drain a large overage — and `MEDIA_CACHE_MIN_ENTRIES` (20) is a floor: the cache is never wiped automatically (only the full-admin purge wipes). The byte total is `SUM(media_cache.size)` in D1 (never a KV counter: concurrent updates lost writes); while the cache is over `MEDIA_CACHE_MAX_BYTES`, fetching and backfill pause. Admin API: `GET/DELETE /api/v1/admin/media_cache`.
