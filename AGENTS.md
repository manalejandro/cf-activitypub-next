...
- **API docs**: `npm run generate:openapi` (runs on predev/prebuild/prepreview/predeploy) writes `lib/api-docs/openapi.json` for the Swagger UI; `npm run check:openapi` fails when the committed spec is stale. The generator is **self-contained** (no metadata file): it scans `app/**/route.ts` and derives paths/methods, tags from the path, query params (`searchParams.get`), request bodies and the file's leading comment as description. Security is read from the handlers: everything under `/api/**/admin` is locked (administrator/moderator role), as is any handler that resolves the caller (`getAuthenticatedActor`, `getAdminRole`, manual `Bearer` parsing) and rejects a missing one (`if (!actor) return unauthorized()`, `role !== "admin"`). Endpoints that only read the viewer optionally (public timelines), login/registration endpoints and HTTP-signature inboxes stay public. Handlers that delegate (`PATCH → PUT`) or are re-exported inherit the target's guard. `lib/__tests__/openapi.test.ts` asserts the committed file matches the source and pins the lock/public matrix.
