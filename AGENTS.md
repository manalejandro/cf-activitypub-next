...
- **Remote resolution requires auth; cached remote content is public**: `/api/v1/accounts/:id` refreshes a remote actor only for authenticated requests (anonymous sees the cached copy, 401 if not cached), `/api/v1/accounts/:id/statuses` only backfills from the remote outbox when authenticated, and `/api/v2/search` remote resolution, `/api/v1/statuses/:id` on-demand resolution and `/api/v1/e2ee/resolve` require auth. `/lookup` only reads the cache so it stays public. Discoverable collections (and their pages/links) are public even for remote accounts.
